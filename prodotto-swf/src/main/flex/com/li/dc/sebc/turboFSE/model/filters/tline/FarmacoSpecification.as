package com.li.dc.sebc.turboFSE.model.filters.tline
{
	...
	public class FarmacoSpecification extends AbstractCompositeSpecification
	{
		...
		public function FarmacoSpecification(listItems:Array)
		{
			super();
		...
		override public function isSatisfiedBy(candidate:Object):Boolean
		{
			if( isNull.not().isSatisfiedBy( listItems ))
			{
				...
				if( isNull.not().isSatisfiedBy( candidate ) )
				{
					...
					//if(isNull.isSatisfiedBy( wrap.farmaco))return true;
					if( isNull.not().and( isArrayC ).isSatisfiedBy( wrap.farmaco ) )
					{
						var farmaci:ArrayCollection =  wrap.farmaco  as ArrayCollection;
						var iter:IIterator = new ArrayIterator(listItems);
						while(iter.hasNext())
						{
							var voice:String = iter.next() as String;
							if(farmaci.contains( voice ))
							{
								return true;
							}
						}
